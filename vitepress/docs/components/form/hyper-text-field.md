...
- 输入容器采用澎湃OS4 风格：无硬描边，默认使用 `HyperColors.elevatedContainer` 半透明玻璃托盘 + `glassHighlightBrush` 顶部高光（与 `HyperIconButton` 一致）；聚焦时在容器上叠一层 `accent` 14% 主题色覆盖层，容器色温融入主题色，不切换硬描边。
