...
`HyperNavBar` 是三段式顶部导航栏容器：`navigationContent`、`titleContent`、`actionContent`。默认背景透明并继承页面底色，组件自身不绘制描边和阴影。
...
`colors()` 默认把 `containerColor` 解析为 `Color.Transparent`，把 `contentColor` 解析为 `HyperColors.primaryText`。组件没有描边或阴影颜色 API。
