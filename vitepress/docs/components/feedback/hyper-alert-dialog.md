...
`HyperAlertDialog` 是基于 `HyperDialog` 的结构化弹窗。它只提供 title、body、action 三个 slot 区域，不内置确认、取消、危险操作等业务按钮，并继承 `HyperDialog` 的默认轻描边面板。
...
- 弹窗不渲染遮罩或半透明蒙层。
- 面板描边由内部 `HyperDialog` 默认值提供；当前 `HyperAlertDialog` 不单独暴露 `border` 参数。
