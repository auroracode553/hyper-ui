...
- 默认图标：Lucide Android `fast-forward`
...
- `leadingContent` 为空时通过 Lucide Android 的 `fast-forward` VectorDrawable 显示快进图标，不再用 Canvas 手绘；传入自定义内容时通过 `LocalContentColor` 接收 `valueColor`。
