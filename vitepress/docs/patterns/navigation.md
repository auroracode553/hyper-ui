...
`HyperTabBar` 不依赖任何导航框架。贴底容器默认只使用 0.5dp 低对比度顶部发丝线分层，不使用阴影或整框描边；深色模式下容器和分隔线与当前页面同色，浅色模式保留轻量透明度。组件使用 55dp 标签操作区和 5dp 轻量底部留白，默认总高度为 60dp。需要完全自定义内容时使用完整 slot；需要统一处理点击与选中颜色时使用泛型 items 入口。
