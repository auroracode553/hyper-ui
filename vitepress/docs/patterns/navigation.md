...
`HyperTabBar` 不依赖任何导航框架。组件使用 55dp 标签操作区和 5dp 轻量底部留白，默认总高度为 60dp，让内容与 Android 手势小白条保持一点距离，同时维持紧凑高度。浅色模式保留透明玻璃效果，深色模式使用不透明实色底栏。需要完全自定义按钮、徽标、输入框或快捷功能时，直接使用完整内容 slot；需要统一处理项目点击和选中颜色时，可使用泛型 items 入口。
